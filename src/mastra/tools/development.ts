...
- Tailor this prompt to the specific nature of the user's request (e.g., add specific rules for charts if requested).`,
      ...
      const chat = await generateCode(userEnhancedPrompt, systemPrompt, context.chatId);
...
const generateCode = async (userPrompt: string, systemPrompt: string, chatId?: string) => {
  try {
    let chat;
    if (chatId) {
